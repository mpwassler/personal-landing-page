<script>
  ...
  const pointDistanceToMouse = (mousePoint, [x,y]) => {
    if (!x || !y || !mousePoint) return null
    var a = mousePoint.x - x
    var b = mousePoint.y - y
  ...
  onMount(() => {
    ...
    (function animate() {
      ...
      for (var i = 0, j = 0; i < toZvalues.length; i++, j += 3) {
        let distance = pointDistanceToMouse(mousePoint3D, [vertices[j], vertices[j + 1]])
        if (distance) {
